...
public class MemberDAO2 {
	...
	public MemberDAO2() {  //데이터베이스의 특정 테이블에 접근하는 이 DAO 객체가 만들어지는 동시에 연결 pool도 생성
		pool = DBConnectionMgr.getInstance();  //이 DAO2 클래스에 대해 객체 생성시 연결된 커넥션 생성해주기. 이미 연결 pool이 존재한다면 해당 객체주소값 가져옴
	}
	...
	public MemberDTO select(MemberDTO dto) throws Exception {
		...
		pool.freeConnection(con,ps,rs);  //임대받은 연결을 돌려주면서 다른 것도 다 같이 close 가능
		return dto2;
	}
