...
<script type="text/javascript">
	$(function() {
		$("#member").on("submit", function() { //member에 대해 submit한 경우 아래의 함수 내용을 실행시켜주세요
			...
			//어떤 데이터를 일단 서버에 넘기는거면 이 serialize()를 해줘야 하는건가
		...
		$("#btn2").on("click", function() {
			$.ajax({
				url: "resources/txt/rank.json",		//목적지 주소 기입(가져올 파일. 외부의 URL주소여도 된다.)
				dataType: "json",
				success: function(result) { //json 문서가 result에 들어가있음
					...
					if(list.length > 0 ){
						alert("json ok..");
						$(list).each(function(i, person) { //for each. i는 뭐인걸까? 인덱스??
							...
							다만 받아온 값에 대해 서버로 데이터로 묶어 다시 보낼 때에는 serialize가 필요할지도
							*/
